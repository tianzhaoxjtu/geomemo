...
export function ExperienceBreakdownPanel({
  stats,
  layout = "vertical",
  className = "",
}: ExperienceBreakdownPanelProps) {
  const { t } = useI18n();
  ...
  return (
    <SurfaceCard
      ...
      className={className}
    >
      <div className={isHorizontal ? "grid gap-3 md:grid-cols-3" : "space-y-3"}>
        <BreakdownPill
...
function BreakdownPill({
  ...
  return (
    <div className="rounded-[24px] border border-white/80 bg-gradient-to-b from-white/95 to-slate-50 px-4 py-4 shadow-sm shadow-slate-900/5">
      ...
      <div className={`mt-3 flex items-end justify-between gap-4 ${horizontal ? "md:min-h-[44px]" : ""}`}>
